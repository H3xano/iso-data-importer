...
RSpec.describe Iso::Data::Importer::Models::TechnicalComittee do # Or TechnicalCommittee
  ...
  context 'when initialized with rich data (e.g., ISO/IEC JTC 1, ID: 45020)' do
    ...
    describe 'nested :title attribute' do
      it { expect(committee.title).to be_an_instance_of(Iso::Data::Importer::Models::LocalizedString) }
      it { expect(committee.title.en).to eq("Information technology") }
      it { expect(committee.title.fr).to be_nil } # Assuming 'fr' key absent in fixture for this title
    end

    describe 'nested :secretariat attribute' do
      it { expect(committee.secretariat).to be_an_instance_of(Iso::Data::Importer::Models::OrganizationReference) }
    ...
    describe 'nested :scope attribute' do
      it { expect(committee.scope).to be_an_instance_of(Iso::Data::Importer::Models::LocalizedString) }
      it { expect(committee.scope.en).to eq("Standardization in the field of information technology.") }
    end
    ...
    describe ':p_members collection' do
      it { expect(committee.p_members).to all(be_an_instance_of(Iso::Data::Importer::Models::OrganizationReference)) }
      it { expect(committee.p_members.size).to be > 5 } # Expect several members
      it 'contains expected member data' do
        expect(committee.p_members.map(&:acronym)).to include("ANSI", "DIN", "SCC")
      end
    end

    describe ':o_members collection' do
      it { expect(committee.o_members).to all(be_an_instance_of(Iso::Data::Importer::Models::OrganizationReference)) }
    ...
    describe ':committee_liaisons collection' do
      it { expect(committee.committee_liaisons).to all(be_an_instance_of(Iso::Data::Importer::Models::OrganizationReference)) }
      it { expect(committee.committee_liaisons.size).to be > 5 }
      it 'correctly maps data, including null references' do
        liaison_with_null_ref = committee.committee_liaisons.find { |l| l.id == 55172 } # ID from fixture with null reference
        expect(liaison_with_null_ref&.reference).to be_nil if liaison_with_null_ref

        liaison_with_ref = committee.committee_liaisons.find { |l| l.id == 54110 } # ID with a reference
        expect(liaison_with_ref&.reference).to eq("ISO/TC 184") if liaison_with_ref
      end
    end

    describe ':organization_liaisons collection' do
      it { expect(committee.organization_liaisons).to all(be_an_instance_of(Iso::Data::Importer::Models::OrganizationReference)) }
      it { expect(committee.organization_liaisons.size).to be >= 4 }
      it 'contains expected liaison data' do
        expect(committee.organization_liaisons.map(&:acronym)).to include("EC - European Commission", "ITU")
      end
    end
  ...
  context 'when initialized with data containing many nulls (e.g., IEC/ISO JSyC BDC, ID: 10320021)' do
    ...
    describe ':p_members collection (with one member)' do
      ...
      it { expect(committee.p_members.first).to be_an_instance_of(Iso::Data::Importer::Models::OrganizationReference) }
     ...
     describe ':committee_liaisons collection (with one liaison)' do
      ...
      it { expect(committee.committee_liaisons.first).to be_an_instance_of(Iso::Data::Importer::Models::OrganizationReference) }
  ...
  context 'when initialized with JSON data where optional keys are absent' do
    let(:absent_keys_hash) { JSON.parse('{"id":999,"reference":"ISO/TC TEST","status":"Active"}') }
